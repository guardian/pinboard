...
export const searchMentionableUsers = async (
  ...
  const groupsPromise = sql`
    ...
    LIMIT 3
  `;
  const chatBotsPromise = Promise.resolve(
    [
      //TODO list all the chatBots from paramstore in playground account???
      {
        shorthand: "chatGuPT",
        description: "sample chat bot",
      },
      {
        shorthand: "HelloWorld",
        description: "replies to hello with world",
      },
    ].filter((chatBot) =>
